...
# Naive Bayes, Logical Regression, Random Forest, and SVM Algorithms
...
def MainProcessCount(path):				# Count Model Extraction
	df = pd.read_csv(path)
	print(df.head())
	y = df.label
	df.drop("label", axis=1)      #where numbering of news article is done that column is dropped in dataset
	X_train, X_test, y_train, y_test = train_test_split(df['text'], y, test_size=0.33, random_state=53, shuffle=True)
...
def MainProcessTfidf(path):				# TFIDF Model Extraction
	df = pd.read_csv(path)
	print(df.head())
	y = df.label
	df.drop("label", axis=1)      #where numbering of news article is done that column is dropped in dataset
...
def MainProcessNgram(path):					#Ngram model
	df = pd.read_csv(path)
	df = df.sample(frac=1)
	print(df.head())
	y = df.label
